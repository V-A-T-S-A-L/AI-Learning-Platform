  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "assistant",
      content: `Hello! I'm your AI learning assistant for "${document.title}". Ask me any questions about this document, and I'll help you understand the content better.`,
      timestamp: new Date(),
    },
  ])
          ...
          {messages.map((message) => (
            <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`flex max-w-[85%] ${message.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                <div
                  className={`flex items-center justify-center h-10 w-10 rounded-2xl flex-shrink-0 ${
                    message.role === "user" ? "bg-purple-600 ml-3" : "bg-zinc-800 mr-3"
                  }`}
                >
                  {message.role === "user" ? (
